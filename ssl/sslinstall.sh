...
if [ "$(ls -A /ssl/root)" ]; then
  for cert in /ssl/root/*
    do
      echo "Try to add file: $cert .. \n"
      $JAVA_HOME/bin/keytool -keystore $JAVA_HOME/jre/lib/security/cacerts -importcert -storepass changeit -noprompt -alias $cert -file $cert
      if [ "$?" -ne "0" ];
        then
          printf "something wrong\n"
        else
          printf "All ok, $cert added to rootcas\n"
      fi
  done
fi
